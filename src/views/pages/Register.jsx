...
export default function RegisterPage() {
  ...
  const { signUp, signInWithGoogle, signInWithAzure } = useAuth();
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      setErrors((prev) => ({ ...prev, email: 'アカウント作成に失敗しました' }));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        setErrors((prev) => ({ ...prev, email: 'Googleアカウント登録に失敗しました' }));
      }
  ...
  const handleMicrosoftSignUp = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        setErrors((prev) => ({ ...prev, email: 'Microsoftアカウント登録に失敗しました' }));
      }
      // OAuth will redirect automatically
    } catch (error) {
      console.error('Unexpected Microsoft sign-up error:', error);
      setErrors((prev) => ({ ...prev, email: 'Microsoftアカウント登録に失敗しました' }));
    } finally {
      ...
      <div className="flex min-h-screen w-full">
        ...
        <div className="flex w-full items-center justify-center bg-white px-6 py-12 lg:w-[45%] lg:px-12">
          <div className="w-full max-w-[420px]">
            ...
            {/* Title */}
            <div className="mb-6 text-center">
              <h1 className="mb-2 text-3xl font-bold">アカウント作成</h1>
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              <div>
                <div className="flex items-start">
                  <input
                    ...
                    className="form-check-input input-primary mt-1"
                  />
                  <label htmlFor="terms" className="ml-2 cursor-pointer text-sm text-gray-700">
                    <NextLink href="/terms" className="text-primary-500 hover:underline">
                      利用規約
                    </NextLink>
                    と
                    <NextLink href="/privacy" className="text-primary-500 hover:underline">
                      プライバシーポリシー
                    </NextLink>
                    に同意します
                  </label>
              ...
              <button
                ...
                aria-busy={isLoading}
                className="btn btn-primary mt-6 flex w-full items-center justify-center gap-2 shadow-lg disabled:cursor-not-allowed disabled:opacity-50"
              >
                {isLoading ? (
                  <>
