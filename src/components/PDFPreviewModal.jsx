...
export default function PDFPreviewModal({ patientId, isOpen, onClose, pdfPreviewUrl, onPreviewReady }) {
  ...
  const handleMobileDownload = async () => {
    ...
    try {
      ...
      const url = `${BACKEND_URL}/pdf/patient-record/${patientId}`;
      ...
      link.href = blobUrl;
      link.download = `patient_record_${patientId}.pdf`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      window.URL.revokeObjectURL(blobUrl);
    } catch (err) {
      console.error('Mobile PDF download error:', err);
  ...
  const handleMobilePreviewToggle = async () => {
    // If already showing preview, close it
    if (pdfPreviewUrl) {
      onPreviewReady(null);
      return;
    }
    ...
    try {
      ...
      console.error('Mobile PDF preview error:', err);
      ...
      <div className="fixed inset-0 z-50 bg-white flex flex-col overflow-x-hidden" style={{ boxSizing: 'border-box' }}>
        ...
        <div className="flex items-center justify-between px-3 py-2.5 border-b border-gray-200 bg-white flex-shrink-0" style={{ width: '100%', maxWidth: '100%', boxSizing: 'border-box' }}>
          <div className="flex items-center gap-2.5 flex-shrink min-w-0">
            ...
            <h3 className="text-base font-semibold text-gray-900 truncate">PDF操作</h3>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors flex-shrink-0"
