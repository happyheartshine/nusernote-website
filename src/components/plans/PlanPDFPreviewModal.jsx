...
/**
 * Full-screen PDF Preview Modal for Plans
 * Desktop: Shows inline, Mobile: Full-screen modal
 */
export default function PlanPDFPreviewModal({ planId, isOpen, onClose, pdfPreviewUrl, onPreviewReady }) {
  ...
  // Mobile-only: inline download handler (mini icon button)
  const handleMobileDownload = async () => {
    ...
    try {
      ...
      const url = `${BACKEND_URL}/plans/${planId}/pdf`;
      ...
      link.href = blobUrl;
      link.download = `plan_${planId}.pdf`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      window.URL.revokeObjectURL(blobUrl);
    } catch (err) {
      console.error('Mobile plan PDF download error:', err);
  ...
  // Mobile-only: inline preview handler (mini icon button)
  const handleMobilePreviewToggle = async () => {
    ...
    try {
      ...
      console.error('Mobile plan PDF preview error:', err);
    ...
    <div className="border-t border-gray-200 p-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <PDFDownloadButton
          ...
          filename={`plan_${planId}.pdf`}
          method="GET"
        />
        <PDFPreviewButton
          label={pdfPreviewUrl ? '計画書 プレビューを閉じる' : '計画書をプレビュー'}
          endpoint={`/plans/${planId}/pdf`}
